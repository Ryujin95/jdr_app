...
export default function RightSidebar({ open, setOpen }) {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        addNotification?.({ type: "error", message: e?.message || "Impossible de charger la liste d'amis." });
      } finally {
  ...
  const onlineUsers = useMemo(() => friends.filter((f) => !!f.isOnline), [friends]);
  const offlineUsers = useMemo(() => friends.filter((f) => !f.isOnline), [friends]);
  const incoming = useMemo(() => Array.isArray(requests?.incoming) ? requests.incoming : [], [requests]);
  const outgoing = useMemo(() => Array.isArray(requests?.outgoing) ? requests.outgoing : [], [requests]);
  ...
  async function onAccept(friendshipId) {
    ...
    try {
      ...
      addNotification?.({ type: "error", message: e?.message || "Impossible d'accepter la demande." });
    }
  }
  ...
  async function onDecline(friendshipId) {
    ...
    try {
      ...
      addNotification?.({ type: "error", message: e?.message || "Impossible de refuser la demande." });
    }
  }
  ...
  async function onCancel(otherUserId) {
    ...
    try {
      ...
      addNotification?.({ type: "error", message: e?.message || "Impossible d'annuler." });
    }
  }
  ...
  function openFriendMenu(e, friend) {
    ...
    setCtxMenu({ x: e.clientX, y: e.clientY, friend: { userId: friend.userId, username: friend.username } });
  }
  ...
  useEffect(() => {
    ...
    const onKeyDown = (ev) => { if (ev.key === "Escape") closeFriendMenu(); };
  ...
  return (
    ...
    >
      <div className="rs-block">
            ...
            {incoming.map((r) => (
              <div key={String(r.friendshipId)} className="rs-req-row">
                ...
                <div className="rs-req-actions">
                  <button type="button" className="rs-req-btn ok" onClick={() => onAccept(r.friendshipId)}>Accepter</button>
                  <button type="button" className="rs-req-btn no" onClick={() => onDecline(r.friendshipId)}>Refuser</button>
                </div>
              </div>
            ))}
          </div>
        ...
        {outgoing.length > 0 && (
          <div className="rs-req-block rs-req-block--outgoing">
            <div className="rs-subtitle">En attente</div>
            {outgoing.map((r) => (
              <div key={String(r.friendshipId)} className="rs-req-row">
                <div className="rs-req-name">{r.username}</div>
                <div className="rs-req-actions">
                  <button type="button" className="rs-req-btn ghost" onClick={() => onCancel(r.userId)}>Annuler</button>
                </div>
              </div>
            ))}
          </div>
      ...
      {ctxMenu && (
        ...
        >
          <button
            ...
            onClick={() => { navigate(`/friends/${ctxMenu.friend.userId}`); closeFriendMenu(); }}
          >
            Profil
          </button>
          <button
            ...
            onClick={async () => {
              ...
              try {
                ...
                closeFriendMenu();
                addNotification?.({ type: "error", message: e?.message || "Impossible de retirer cet ami." });
              }
            }}
          >
            Retirer des amis
