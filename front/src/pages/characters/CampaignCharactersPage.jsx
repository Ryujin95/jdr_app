...
function CampaignCharactersPage() {
  ...
  const handleOpenCard = async (charId) => {
    ...
    if (!knownMap[charId]) await loadKnown(charId);
  };
  ...
  useEffect(() => {
    if (!token || !myPlayerCharacterId || knownMap?.[myPlayerCharacterId]) return;
  ...
  const relationByToId = useMemo(() => {
    ...
    for (const r of list) {
      const toId = r?.id ?? r?.toCharacter?.id ?? r?.toCharacterId ?? r?.to_character_id ?? r?.toId ?? null;
  ...
  return (
    <div className="characters-page">
      <div className="characters-page-header">
        <h1 className="characters-page-title">Personnages</h1>
            ...
            {clanCharacters.map((char) => {
              ...
              return (
                <CharacterCard
                  ...
                  affinityScore={rel?.score ?? null}
                  affinityType={rel?.type ?? null}
      ...
      {createOpen && (
        ...
        >
          <div className="modal-content modal-content--large" onMouseDown={(e) => e.stopPropagation()}>
            <CampaignEditorPage
              embed
