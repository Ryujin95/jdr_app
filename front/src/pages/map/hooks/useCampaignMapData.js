...
import { apiListMaps } from "../../../api/api";
...
export function useCampaignMapData({ token, campaignId }) {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const data = await apiListMaps(token, campaignId, { signal: controller.signal });
        const list = Array.isArray(data) ? data : [];
        setMaps(list);
        if (list.length > 0) {
          setSelectedMapId((prev) =>
            prev && list.some((m) => String(m.id) === String(prev)) ? prev : list[0].id
          );
        }
      } catch (e) {
  ...
  const createLocation = useCallback(async ({ name, description }) => {
    if (!token || !campaignId) return;
  ...
  const deleteLocationToTrash = useCallback(async (locationId) => {
    if (!token || !campaignId) return;

    const id = String(locationId || "");
    if (!id) throw new Error("Choisis un lieu à supprimer.");

    const res = await fetch(`${API_URL}/trash/move/location/${encodeURIComponent(id)}`, {
      method: "PATCH",
      headers: {
        Authorization: `Bearer ${token}`,
        "Content-Type": "application/json",
  ...
  const refreshZones = useCallback(async () => {
    if (!token || !selectedMap?.id) return;
  ...
  const moveCharacterToZone = useCallback(async (characterId, targetZone) => {
    if (!token) throw new Error("Non authentifié");
    if (!characterId) throw new Error("characterId manquant");

    const locationId = targetZone?.locationId ?? targetZone?.location_id ?? targetZone?.location?.id ?? null;
    if (!locationId) throw new Error("La zone cible n'a pas de lieu associé.");
  ...
  return {
    ...
    deleteZone, moveCharacterToZone,
  };
}