...
export default function MapCanvas({
  ...
  return (
    <div
      className={`map-container map-container--relative${isEditing && isMjInThisCampaign ? " editing" : ""}`}
    ...
    >
      ...
      {!loadingZones && (Array.isArray(zones) ? zones : []).map((z) => {
        ...
        const chars = z?.characters ?? z?.personnages ?? z?.persons ?? z?.members ?? z?.zoneCharacters ?? z?.zone_characters ?? [];
        const list = Array.isArray(chars) ? chars : [];

        return (
          <div
            key={z.id}
            className={`map-zone${activeZoneId === z.id ? " active" : ""}`}
            style={{
              top: `${top}%`,
              left: `${left}%`,
              width: `${width}%`,
              ...
              <div className="map-zone-characters">
                {list.map((c) => {
                  ...
                  return (
                    ...
                    >
                      {avatar && <img className="map-zone-character-avatar" src={avatar} alt={nickname || "avatar"} draggable={false} />}
                      {nickname && <div className="map-zone-character-nickname">{nickname}</div>}
                    </div>
                  );
                })}
              </div>
